 ...
 * Model 构造函数
 *
 * 提供MongoDB集合的接口以及创建文档实例。
 *  
 * @param {Object} doc 值用于创建文档
 * @inherits Document http://mongoosejs.com/docs/api.html#document-js
 * @event `error`: 如果正在侦听此事件,在保存文档而不传递回调并发生“错误”时会发出错误,如果没有监听,则事件会冒泡到创建此 Model 的链接.
 ...
 * 继承自 Document.
 *
 * 所有 Model.prototype 特征都可以用顶级（非子）文件。
 */
...
/**
 * 当调用“save()”和“isNew”是“false”时附加到查询的附加属性。
 *
 * @api public
 * @property $where
 * @memberOf Model
...
/**
 * 如果这是一个鉴别器model，“baseModelName”是基础model名称。
 *
 * @api public
 ...
 *
 * 1. 如果发生错误时为`err` 
 * 2.  当保存 `product`时为`product`
 *
 * 作为历程控制的额外措施, 保存将返回一个Promise.
 * ####示例:
 ...
 * @param {Document} self
 * @param {String} path
 * @return {Boolean} true 当跳过给定路径的版本控制时
 */
...
/*!
 * 确定是否用某种形式的过滤器填充了数组，现在是否以一种可能会意外覆盖数据的方式进行更新。
 *
 ...
 *
 *
 * 作为流量控制的一种额外措施，remove将返回一个Promise（如果通过则绑定到`fn`），因此可以将其链接或挂钩以接收错误。
 *
 * ####Example:
...
/**
 * 对MongoDB执行此模型的任何异步初始化。
 * 目前，此功能仅负责建立 [indexes](https://docs.mongodb.com/manual/indexes/),
 * 除非 [`autoIndex`](http://mongoosejs.com/docs/guide.html#autoIndex) is turned off.
 *
 * 此功能会自动调用，因此您无需调用它。
 * 这个函数也是幂等的，所以您可以调用它来获得一个承诺，这个承诺将在您的索引构建完成时解析，作为' MyModel.on('index') '的替代。
 *
 * ####Example:
 *
