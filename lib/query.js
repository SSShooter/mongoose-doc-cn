...
/**
 * Query 构造函数，用来构建查询器（query），不过并不需要直接实例化 `Query` 对象，而是用 Model 的 [`Model.find()`](/docs/api.html#find_find) 这些函数。
 *
 * ####示例:
 *
 ...
 * @memberOf Query
 * @param {String} [path]
 * @param {Array} val 长度必须是2，第一个元素是 `divisor`，第二个是 `remainder`.
 * @return {Query} this
 ...
 * ####注意:
 *
 * 在 Mongoose 3.7 中，`within` 由 getter 改变成了 function。如果你需要旧语法，查看 [这里](https://github.com/ebensing/mongoose-within)。
 *
 * @method within
 ...
 * 最后，如果路径有前缀 `+`，将被强制选择，这对于在 [schema level](/docs/api.html#schematype_SchemaType-select) 被排除的路径会有用。
 *
 * 映射 _必须_ 只能是 包含 或 排除 二者其一。换句话说，只能要么列举包含的字段（将排除其他所有字段），要么列举排除的字段（将选择其他所有字段）。
 ...
 * 设置 lean 选项
 *
 * 那些激活 `lean` 选项的查询，返回的文档是普通 javascript 对象，而不是 [Mongoose Documents](#document-js)。
 * 这些对象没有 `save` 方法、getters/setters，也没有被赋予其他 Mongoose magic。
 *
 * ####示例:
 *
 *     new Query().lean() // true
 ...
 *     });
 *
 * 在高性能只读场景下这个选项就 [厉害](https://groups.google.com/forum/#!topic/mongoose-orm/u2_DzDydcnA/discussion) 了，特别是跟 [stream](#query_Query-stream) 组合使用时。
 *
...
/**
 * 读取/赋值 query 的 error 标识。如果标识不是 null 或
 ...
 *     });
 *
 * @param {Error|null} err 如果被赋值，`exec()` 会在查询被发送到 MongoDB 之前速效失败
 * @returns {Query} this
 * @api public
 */
 ...
 *
 * @param {Object} [conditions] mongodb selector
 * @param {Function} [callback] 可选 参数表是 (error, count)
 * @return {Query} this
 * @see count http://docs.mongodb.org/manual/reference/method/db.collection.count/
 * @api public
 */
 ...
 * @param {Function} [callback] 可选 参数表是 (error, arr)
 * @return {Query} this
 ...
 * @param {Function} [callback] 可选 参数表是 (error, writeOpResult)
 * @return {Query} this
 ...
 * @param {Boolean} [options.multipleCastError] 默认情况下 mongoose 只返回第一个 cast 错误。激活该选项来聚合所有的 cast 错误。
 ...
 * @param {Function} [callback] 可选 参数表是 (error, doc), _除非_ 设置了 `rawResult` 选项，那么参数将是 (error, writeOpResult)
 ...
 *
 * _被传入的路径中非原子（$atomic）的操作，会以 $set 形式执行。_
 *
 * 这个函数触发以下中间件
 *
 * - `update()`
...
/**
 * 声明且（/或）执行当前查询为 updateMany() 操作。不同于
 * `update()` 的是，MongoDB 会忽略 `multi` 选项，_只_ 更新第一条匹配
 * `criteria` 的文档。
 *
 * **注意** updateOne _不会_ 触发 update 中间件。可以用 `pre('updateOne')`
 * 和 `post('updateOne')` 代替。
 *
 * 这个函数触发以下中间件
 *
 ...
 * @param {Object} [options]
 * @param {Boolean} [options.multipleCastError] 默认情况下，mongoose 只返回 query 转换中报的第一个错。激活该选项会聚合转换中所有的报错。
 * @param {Function} [callback] 参数表是(error, writeOpResult)
 * @return {Query} this
 * @see Model.update #model_Model.update
 * @see update http://docs.mongodb.org/manual/reference/method/db.collection.update/
 * @see writeOpResult http://mongodb.github.io/node-mongodb-native/2.2/api/Collection.html#~WriteOpResult
...
/**
 * 声明且（/或）执行当前查询为 replaceOne() 操作。不同于
 * `update()` 的是，MongoDB 不接受原子操作符（`$set`等）直接替换存在的文档。
 *
 * **注意** replaceOne _不会_ 触发 update 中间件。可以用 `pre('replaceOne')`
 * 和 `post('replaceOne')` 代替。
 *
 * 这个函数触发以下中间件
 *
 * - `replaceOne()`
 ...
 * @param {Function} [callback] 可选 参数表取决于调用的函数
 * @return {Promise}
 * @api public
 */
 ...
 *
 * @param {Object|String} path 需要做表关联的字段路径，或者包含所有参数的对象
 * @param {Object|String} [select] 表关联查询要选择的字段
 * @param {Model} [model] 表关联的 model 。如果没有指定，将以 Schema 中 `ref` 字段为名称查找 model 进行关联。
 * @param {Object} [match] population 查询的条件
 * @param {Object} [options] population 查询的选项 (sort 等)
...
/**
 * 返回对 [mongodb driver cursor](http://mongodb.github.io/node-mongodb-native/2.1/api/Cursor.html) 的封装。
 * QueryCursor 暴露了一个 Streams3 接口，和一个 `.next()` 函数。
 *
 * `.cursor()` 函数触发 find 的前置钩子，但**不会**触发后置钩子。
 *
 * ####示例
 *
 *     // 有2种方式使用游标。第一种，流式:
 ...
 * 不能和 `distinct()` 一起使用
 *
 * @param {Boolean} bool 默认值 true
 * @param {Object} [opts] 要设置的选项
 * @param {Number} [opts.numberOfRetries] 游标用尽时，重试终止前的次数
 * @param {Number} [opts.tailableRetryInterval] 游标用尽时，重试之前等待的毫秒数
