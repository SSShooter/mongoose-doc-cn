 ...
 * ####Options:
 *
 * - [autoIndex](/docs/guide.html#autoIndex): bool - 缺省值 null （即使用数据库连接自带的 autoIndex 选项）
 * - [bufferCommands](/docs/guide.html#bufferCommands): bool - 缺省值 true
 * - [capped](/docs/guide.html#capped): bool - 缺省值 false
 * - [collection](/docs/guide.html#collection): string - 没有缺省值
 * - [id](/docs/guide.html#id): bool - 缺省值 true
 * - [_id](/docs/guide.html#_id): bool - 缺省值 true
 * - `minimize`: bool - 当 [document#toObject](#document_Document-toObject) 被手动调用时控制其行为 - 缺省值 true
 * - [read](/docs/guide.html#read): string
 * - [safe](/docs/guide.html#safe): bool - 缺省值 true.
 * - [shardKey](/docs/guide.html#shardKey): bool - 缺省值 `null`
 * - [strict](/docs/guide.html#strict): bool - 缺省值 true
 * - [toJSON](/docs/guide.html#toJSON) - object - 没有缺省值
 * - [toObject](/docs/guide.html#toObject) - object - 没有缺省值
 * - [typeKey](/docs/guide.html#typeKey) - string - 缺省值 'type'
 * - [useNestedStrict](/docs/guide.html#useNestedStrict) - boolean - 缺省值 false
 * - [validateBeforeSave](/docs/guide.html#validateBeforeSave) - bool - 缺省值 `true`
 * - [versionKey](/docs/guide.html#versionKey): string - 缺省值 "__v"
 * - [collation](/docs/guide.html#collation): object - 缺省值 null （即不使用 collation 功能）
 *
 * ####注意:
 *
 * _对 schema 进行嵌套时， （上例中的 `children`）， 传给父 schema 引用之前一定要先声明子 schema。_
 *
...
/**
 * 一个数组，包含子 schema （document 数组中的，和单个的嵌套 subdoc） 和对应已编译的 model。每个元素都是个对象，包含2个属性：`schema` 和 `model`。
 *
 * 这个属性通常只有插件开发者和高级用户会使用。
 * 你完全没必要操作它。
 *
 * @api public
...
/**
 * 返回该 schema 的缺省选项，返回值已经合并了 `options` 参数。
 *
 * @param {Object} options
 * @return {Object}
 * @api private
 ...
 *
 * _注意:_ 允许使用这些名字作为方法名，但是自负风险， 因为它们可能是 mongoose document 中已经存在的方法名。
 *
...
/**
 * 返回字段上的索引和 schema-level 的索引（已被缓存的）
 *
 * @api private
 * @return {Array}
 */
 ...
 * 向方法调用的队列中增加一条
 *
 * @param {String} name 待调用方法的名称
 * @param {Array} args 传给方法的参数 (arguments)
 * @api public
 */
 ...
 *
 * @param {Object} fields
 * @param {Object} [options] 传给 [MongoDB 驱动程序的 `createIndex()` 函数](http://mongodb.github.io/node-mongodb-native/2.0/api/Collection.html#createIndex) 的选项
 * @param {String} [options.expires=null] Mongoose 语法糖， 用 [ms](https://www.npmjs.com/package/ms) 把 `expires` 选项转换成上面链接中 `expireAfterSeconds` 选项所需要的秒格式。
 * @api public
 */
 ...
 *
 * @param {String} key 选项的名字
 * @param {Object} [value] 如果没有传值，选项的当前值被返回
 * @see Schema ./
 * @api public
 */
...
/**
 * 向 schema 中加载一个 ES6 类。把类中的 [setters](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/set) + [getters](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/get),
 * [static methods](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes/static),
 * 和 [instance methods](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes#Class_body_and_method_definitions)
 * 映射成为 schema 中的 [virtuals](http://mongoosejs.com/docs/guide.html#virtuals),[statics](http://mongoosejs.com/docs/guide.html#statics),
 * 和 [methods](http://mongoosejs.com/docs/guide.html#methods)。
 *
 * ####示例:
 *
 * ```javascript
 ...
 * @param {Boolean} [virtualsOnly] 如果值为 true，从类中只加载 virtual 字段，忽略实例方法和静态方法。
 */
