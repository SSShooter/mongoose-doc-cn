...
/**
 * 子 schema 和对应已编译 model 的数组。每个元素都是个对象，包含2个属性：`schema` 和 `model`。
 ...
 * document 中的保留字段。
 *
 * 在 schema 的声明中禁止使用这个对象中的键作为字段名称，因为跟 mongoose 的功能存在冲突。使用这些名字会抛出错误。
 *
 ...
 *     schema.path('name', Number) // 把路径 `name` 的数据类型改成 Number
 *
 * @param {String} path
...
/**
 * 向该 schema 编译 model 构造的 document 添加一个实例方法
 *
 * ####示例
 *
 ...
 *
 * 如果参数只有一个，是 名称/函数 哈希键值对，那么每一对 名称/函数 都被加做实例方法。
 *
 *     schema.method({
 *         purr: function () {}
 *       , scratch: function () {}
 ...
 *     });
 *
 * 如果参数只有一个，是 名称/函数 哈希键值对，那么每一对 名称/函数 都被加做静态方法。
 *
 ...
 *
 * @param {Object} fields
 * @param {Object} [options] 传给 [MongoDB 驱动的 `createIndex()` 函数](http://mongodb.github.io/node-mongodb-native/2.0/api/Collection.html#createIndex)的选项
...
/**
 * 返回 schema 的索引列表，定义自 `schema.index()` 或者路径选项 `index: true`
 *
 * @api public
 */
